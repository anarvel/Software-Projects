...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String str1sub = str1.substring(0, str1.length() - overlap); // new substring of str1 from beginning to the overlap
        String combination = str1sub + str2; // add the new substring without the overlap to str2
        return combination; // returns only one overlap of the str1 and str2
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        Set<String> temp = strSet.newInstance(); // create new set
        boolean addString = true; // boolean to determine whether to add it
        while (strSet.size() > 0) { // iterate until set is empty
            String currentString = strSet.removeAny();
            if (currentString.contains(str)) { // check if contains str
                addString = false; // if then dont add
            }
            temp.add(currentString); // always add back to temp
        }

        if (addString) { // if addString true
            while (temp.size() > 0) {
                String currentString = temp.removeAny();
                if (!str.contains(currentString)) {
                    strSet.add(currentString); // only add back elements that are not a substring of str
                }
            }
            strSet.add(str);
        } else { // if addString false
            strSet.transferFrom(temp); // keep it the same
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> strSet = new Set1L<>(); // set to put input

        while (!input.atEOS()) { // while input is not at the end of string
            String line = input.nextLine(); // store each line
            addToSetAvoidingSubstrings(strSet, line); // call the substring method to make set
        }
        return strSet; // return finished set
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) { // for loop until the end of the text
            if (text.charAt(i) == '~') { // if there is a ~ at an index
                out.println(); // make a new line
            } else {
                out.print(text.charAt(i)); // if not then keep the text the same
            }
        }
    }
