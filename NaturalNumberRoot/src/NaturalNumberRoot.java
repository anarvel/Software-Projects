...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        // find difference between upper and lower bound
        NaturalNumber diff = new NaturalNumber2(tooHigh);
        diff.subtract(lowEnough);

        // while loop until difference is > 1
        while (diff.compareTo(one) > 0) {
            // find the midpoint
            NaturalNumber mid = new NaturalNumber2(tooHigh);
            mid.add(lowEnough);
            mid.divide(two); // average
            // find midpoint ^ r
            NaturalNumber pow = new NaturalNumber2(mid);
            pow.power(r);

            // if n is less than mid
            if (n.compareTo(pow) < 0) {
                tooHigh.copyFrom(mid); // make tooHigh the mid
            } else { // is n is bigger than mid
                lowEnough.copyFrom(mid); // make lowEnough the mid
            }
            // update the difference
            diff.copyFrom(tooHigh);
            diff.subtract(lowEnough); //update max-min
        }
        // set n to the lowEnough
